...
SERVER_THUMBNAIL_URL = "https://i.imgur.com/j7qAkTt.jpeg"
...
WHITELIST_QUESTIONS = [
    ...
    "Δώσε μια σύντομη περιγραφή του χαρακτήρα σου.",
...
STAFF_QUESTIONS = [
    ...
    "Έχεις εμπειρία staff? Αν ναι που?","Τι θα κάνεις αν φίλος σου κάνει abuse perms?",
    "Τι θα κανεις αν member προσβάλει staff?","Τι θα κανεις αν υπάρχουν πολλά άτομα στο support?",
    "Τι βήματα θα ακολουθήσεις αν αναφερθεί περιστατικό in game?",
    "Τι θα κάνεις αν αναφερθεί RDM/VDM?","Γιατί να επιλέξουμε εσένα?"
]
MANAGER_QUESTIONS = [
    "Πόσο χρονών είσαι?","Πώς σε λένε στο roblox?","Πόσες ώρες θα μπορείς να είσαι on duty?",
    "Τι θέση manager θέλεις?","Έχεις εμπειρία management? Αν ναι που?",
    "Τι θα κάνεις αν μέλη τσακώνονται?","Πως θα κρατήσεις ομαδικότητα?",
    "Τι θα κάνεις αν member/staff δεν υπακούει κανόνες?",
...
def is_owner_or_coowner(u): return any(r.id in (FOUNDER_ROLE_ID,OWNER_ID,CO_OWNER_ID) for r in u.roles)
...
async def on_member_update(before, after):
    guild = after.guild; log = bot.get_channel(ROLE_UPDATE_LOG_CHANNEL_ID)
...
class TicketCloseView(discord.ui.View):
    ...
    async def close_ticket(self, interaction, button):
        ...
        if lc:
            e = discord.Embed(title="❌ Ticket Closed", description=f"Έκλεισε από {interaction.user.mention}", color=discord.Color.red())
            e.add_field(name="Channel", value=interaction.channel.mention); await lc.send(embed=e)
...
class MainTicketSelect(discord.ui.Select):
    ...
    async def callback(self, interaction):
        guild=interaction.guild; author=interaction.user; cat=guild.get_channel(MAIN_TICKET_CATEGORY_ID)
        if not cat: return await interaction.response.send_message("Κατηγορία δεν βρέθηκε.",ephemeral=True)
        ow={guild.default_role:discord.PermissionOverwrite(view_channel=False),author:discord.PermissionOverwrite(view_channel=True,send_messages=True,read_message_history=True)}
        v=self.values[0]
        ...
        if lc:
            le=discord.Embed(title="📂 Νέο Ticket",description=f"{author.mention} άνοιξε ticket.",color=discord.Color.blue())
            le.add_field(name="Τύπος",value=tt);le.add_field(name="Channel",value=ch.mention);await lc.send(embed=le)
...
class JobTicketSelect(discord.ui.Select):
    def __init__(self):
        opts=[discord.SelectOption(label="Civilian Job",description="Αίτηση για civilian",emoji="👮"),discord.SelectOption(label="Criminal Job",description="Αίτηση για criminal",emoji="🕵️")]
        super().__init__(custom_id="job_ticket_select",placeholder="Make a selection",min_values=1,max_values=1,options=opts)
    async def callback(self,interaction):
        guild=interaction.guild;author=interaction.user;cat=guild.get_channel(JOB_TICKET_CATEGORY_ID)
        if not cat: return await interaction.response.send_message("Κατηγορία δεν βρέθηκε.",ephemeral=True)
        ow={guild.default_role:discord.PermissionOverwrite(view_channel=False),author:discord.PermissionOverwrite(view_channel=True,send_messages=True,read_message_history=True)}
        ...
        lc=guild.get_channel(TICKET_LOG_ID)
        if lc:
            le=discord.Embed(title="📂 Νέο Job Ticket",description=f"{author.mention} άνοιξε job ticket.",color=discord.Color.blue())
            le.add_field(name="Τύπος",value=tt);le.add_field(name="Channel",value=ch.mention);await lc.send(embed=le)
        await interaction.response.send_message(f"Δημιουργήθηκε: {ch.mention}",ephemeral=True)
...
class ReasonModal(discord.ui.Modal):
    ...
    async def on_submit(self,interaction):
        ...
        if self.orig_msg.embeds:
            oe=self.orig_msg.embeds[0];oe.add_field(name=f"{at} by",value=f"{interaction.user.mention} — {reason}",inline=False);oe.color=color
...
class SendApplicationView(discord.ui.View):
    def __init__(self,app_type,uid,qs,ans): super().__init__(timeout=None); self.app_type=app_type; self.uid=uid; self.qs=qs; self.ans=ans
...
class DutyView(discord.ui.View):
    ...
    async def on_duty(self,interaction,button):
        ...
        duty_data[uid]=duty_data.get(uid,{"total_seconds":0}); duty_data[uid]["start_time"]=time.time(); save_duty_data(duty_data)
    ...
    async def off_duty(self,interaction,button):
        ...
        if uid in duty_data and "start_time" in duty_data[uid]:
            ss=time.time()-duty_data[uid]["start_time"]; duty_data[uid]["total_seconds"]=duty_data[uid].get("total_seconds",0)+ss; duty_data[uid].pop("start_time",None); save_duty_data(duty_data)
...
class ReviewModal(discord.ui.Modal,title="⭐ Make a Review"):
    ...
    async def on_submit(self,interaction):
        ...
        e.add_field(name="Αξιολόγηση",value=sd,inline=False); e.add_field(name="Σχόλιο",value=self.review_input.value,inline=False)
        e.set_author(name=interaction.user.display_name,icon_url=interaction.user.avatar.url if interaction.user.avatar else None)
        e.set_thumbnail(url=SERVER_THUMBNAIL_URL); e.set_footer(text=f"User ID: {interaction.user.id} • Legacy Roleplay")
...
class StarSelect(discord.ui.Select):
    def __init__(self):
        opts=[discord.SelectOption(label="⭐ 1 Αστέρι",emoji="⭐",value="1"),discord.SelectOption(label="⭐⭐ 2 Αστέρια",emoji="⭐",value="2"),
              discord.SelectOption(label="⭐⭐⭐ 3 Αστέρια",emoji="⭐",value="3"),discord.SelectOption(label="⭐⭐⭐⭐ 4 Αστέρια",emoji="⭐",value="4"),
...
async def ban(ctx,member:discord.Member=None,*,reason="No reason"):
    ...
    if not member: return await ctx.reply("Πρέπει να γράψεις ποιον.")
...
async def dmall(ctx,*,message:str):
    fr=ctx.guild.get_role(FOUNDER_ROLE_ID)
    if fr not in ctx.author.roles: return await ctx.reply("❌ Μόνο Founder.")
    sent=0
    for m in ctx.guild.members:
        if m.bot: continue
        try: await m.send(message); sent+=1
        except: continue
    await ctx.reply(f"📨 Στάλθηκε σε **{sent}** μέλη.")
...
async def serverstatus(ctx):
    g=ctx.guild; e=discord.Embed(title="📊 Server Status",color=discord.Color.blurple())
...
async def scan(ctx,member:discord.Member=None):
    ...
    e.set_footer(text=f"Σκανάρισε {guild.member_count} μέλη"); await ctx.send(embed=e)
...
async def suggestionpanel(ctx):
    ...
    e.set_image(url=SERVER_BANNER_URL);e.set_thumbnail(url=SERVER_THUMBNAIL_URL);e.set_footer(text="Legacy Roleplay • Suggestion System")
